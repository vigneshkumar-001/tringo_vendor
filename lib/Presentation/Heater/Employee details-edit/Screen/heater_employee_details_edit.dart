  ...
  class _HeaterEmployeeDetailsEditState
    ...
    //   ref.read(heaterEmployeeEditNotifier.notifier).resetLatest();
    ...
    //   //  app recents/back-from-minimize -> refresh employee details
    ...
    void initState() {
      ...
      // ✅ delay provider modifications until first frame is done
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
    ...
    // // ✅ SINGLE SOURCE OF TRUTH (Fix for stale "Block" showing after app reopen)
    // bool _isBlockedFinal() {
    //   final editState = ref.read(heaterEmployeeEditNotifier);
    //   final detailsState = ref.read(heaterEmployeeDetailsNotifier);
    //
    //   // NOTE: change this path if your model differs
    //   final employee = detailsState.employeeDetailsResponse?.data?.employee;
    //   final serverIsActive = employee?.isActive;
    //
    //   final isActiveFinal =
    //       editState.latestIsActive ?? serverIsActive ?? (widget.isActive ?? true);
    //
    //   return !isActiveFinal;
    // }
    bool _isBlockedFinal() {
      final editState = ref.read(heaterEmployeeEditNotifier);
      final detailsState = ref.read(heaterEmployeeDetailsNotifier);
      ...
      // ✅ use latest only if it belongs to THIS employeeId
      final bool? localLatest =
      (editState.latestEmployeeId == widget.employeeId)
          ? editState.latestIsActive
          : null;

      final isActiveFinal = localLatest ?? serverIsActive ?? (widget.isActive ?? true);
      return !isActiveFinal;
    }
    ...
    Future<void> _onBlockUnblockTap() async {
      final editState = ref.read(heaterEmployeeEditNotifier);
      if (editState.isBlockLoading) return;

      // ✅ use final truth (not widget.isActive alone)
      final blockedNow = _isBlockedFinal();

      final ok = await _confirmDialog(
        title: blockedNow ? "Unblock employee?" : "Block employee?",
        message:
            blockedNow
        ...
        yesText: blockedNow ? "Unblock" : "Block",
      );
      ...
      AppSnackBar.success(
        context,
        (s.latestIsActive ?? true) ? "Employee unblocked" : "Employee blocked",
      );

      // ✅ refresh details so serverIsActive updated
      ref
          .read(heaterEmployeeDetailsNotifier.notifier)
          .heaterEmployee(employeeId: widget.employeeId);
    }
    ...
    Widget build(BuildContext context) {
      ...
      // ✅ Use final truth everywhere
      final blocked = _isBlockedFinal();
                    ...
                    children: [
                      ...
                      AnimatedSwitcher(
                        ...
                        child: OwnerVerifyField(
                          ...
                          onSendOtp: (mobile) {
                            return ref
                                .read(heaterEmployeeEditNotifier.notifier)
                                .employeeUpdateNumberRequest(phoneNumber: mobile);
                          },
                      ...
                      CommonContainer.button(
                        ...
                        onTap: () async {
                          ...
                          final newState = ref.read(heaterEmployeeEditNotifier);
...
//   // }
//
//   bool _isBlocked(heaterEmployeeEditState state) {
//     final isActive = state.latestIsActive ?? (widget.isActive ?? true);
//     return !isActive;
//   }
//
//   Future<bool> _confirmDialog({
...
//     if (editState.isBlockLoading) return;
//
//     final blockedNow = _isBlocked(editState);
//
//     final ok = await _confirmDialog(
//       title: blockedNow ? "Unblock employee?" : "Block employee?",
//       message:
...
//
//     //  No manual toggle.
//     // UI automatically uses state.latestIsActive
//
//     AppSnackBar.success(
//       context,
//       (s.latestIsActive ?? true) ? "Employee unblocked" : "Employee blocked",
//     );
//
//     // IMPORTANT: Refresh employee details screen/provider so next time also correct
//     ref
//         .read(heaterEmployeeDetailsNotifier.notifier)
//         .heaterEmployee(employeeId: widget.employeeId);
//   }
...
//     final a = widget;
//
//     // final blocked = _isBlocked(state);
//     //
//     // final editState = ref.watch(heaterEmployeeEditNotifier);
//     //
//     //
//     // final serverIsActive = editState.employeeUnblockResponse?.data?.isActive;
//     //
//     // final isActiveFinal =
//     //     editState.latestIsActive ??
//     //         serverIsActive ??
...
//     //
//     // final blocked = !isActiveFinal;
//     //
//
//     final editState = ref.watch(heaterEmployeeEditNotifier);
//     final detailsState = ref.watch(heaterEmployeeDetailsNotifier);
//
//     //  employee details API response la irundhu isActive edunga
//     // (இந்த path உங்க model structure படி change பண்ணலாம்)
//     final employee = detailsState.employeeDetailsResponse?.data?.employee;
//     final serverIsActive = employee?.isActive;
//
//     //  Final truth
